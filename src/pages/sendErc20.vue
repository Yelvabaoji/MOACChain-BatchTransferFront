<template>
  <q-page padding>
    ...
    <q-form @submit="onSubmit" class="q-gutter-md">
      ...
      <div class='q-mt-none'>
        <p>币名：{{token.name}}，符号：{{token.symb}}，数量：{{String(erc20Balance.toString()).replace(/^(-?)(\d+)((\.\d+)?)$/,
          function (s, s1, s2, s3) {return s1 + s2.replace(/\d{1,3}(?=(\d{3})+$)/g, '$&,') + s3})
        }}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getWalletInfo () {
      ...
      if (this.isERC20) {
        if (this.erc20Address.length !== 42) {
          this.$q.dialog({ title: '警告', message: 'ERC20币合约地址长度不对' })
          return
        }
        // if address is not start from 0x
        if (this.erc20Address.substr(0, 2) !== '0x') {
          this.$q.dialog({ title: '警告', message: 'ERC20币合约地址没有0x开头' })
          return
        }
      }
      ...
      // 获取ERC20代币的信息
      if (this.isERC20) {
        this.token = this.GLOBAL.getTokenInfo(this.chain3, this.erc20Address)
        var bal = await this.GLOBAL.getTokenBalance(this.chain3, this.erc20Address, this.address, this.token.decimals)
        this.erc20Balance = bal.toFixed(2)
      }
    },
    ...
    async onSubmit () {
      ...
      if (this.balance <= (0.0025 * this.totalAccount)) {
      ...
      if (this.erc20Balance <= this.totalAmount) {
        this.$q.dialog({ title: '警告', message: '账户里没有足够的ERC20代币' })
        return
      }
      ...
      this.$q.dialog({ title: '确定',
        ...
        function iterator (i) {
          ...
          // 准备签名
          var contract = that.GLOBAL.getContract(that.chain3, that.erc20Address)
          var data = contract.transfer.getData(that.data[i].addr, that.data[i].amount * (10 ** that.token.decimals))
